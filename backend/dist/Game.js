    ...
    makeMove(ws, move) {
        if (ws === this.player1 || ws === this.player2) {
            ...
            if (this.board.isDraw()) {
                this.player1.emit(JSON.stringify({ type: message_1.GAME_OVER, payload: { winner: "draw" } }));
                this.player2.emit(JSON.stringify({ type: message_1.GAME_OVER, payload: { winner: "draw" } }));
                return;
            }
